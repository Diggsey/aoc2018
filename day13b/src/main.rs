...
fn main() {
    ...
    let mut crashed_carts = 0;
    loop {
        carts.sort_by_key(|cart| cart.pos);
        for i in 0..carts.len() {
            let mut cart = carts[i];
            ...
            if grid[cart.pos.0][cart.pos.1].occupied {
                ...
                for j in 0..carts.len() {
                    if j != i && carts[j].pos == cart.pos && !carts[j].crashed {
                        found_j = Some(j);
                        break;
                    }
                }
                let j = found_j.expect("Crashed with non-existent cart?");
                grid[cart.pos.0][cart.pos.1].occupied = false;
        ...
        if crashed_carts == carts.len() - 1 {
            for cart in carts {
                if !cart.crashed {
                    println!("Remaining cart: {}, {}", cart.pos.1, cart.pos.0);
                }
            }
            break;
        }
