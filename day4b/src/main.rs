...
fn main() {
    ...
    let (guard_id, minute, max_count) = guards.iter()
        .map(|(guard_id, guard)| {
            let (minute, max_count) = guard.counts.iter().enumerate().max_by_key(|(_, &v)| v).unwrap();
            (guard_id, minute as i32, max_count)
        })
        .max_by_key(|&(_, _, v)| v).unwrap();
    
    println!("{} {}", guard_id * minute, max_count);
}
