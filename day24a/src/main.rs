...
fn fight_round(groups: &mut [Group]) {
    // Reset round
    ...
    target_ordering.sort_by_key(|&index| groups[index].effective_power());
...
fn main() {
        ...
        .map(Result::unwrap)
    {
        ...
        groups.push(parse_group(current_side, &line, side_index));
        side_index += 1;
    }
    ...
    loop {
        println!();
        if !groups.iter().any(|group| group.num_units > 0 && group.side == Side::ImmuneSystem) { break; }
        if !groups.iter().any(|group| group.num_units > 0 && group.side == Side::Infection) { break; }
        fight_round(&mut groups);
    }

    let remaining: i32 = groups.iter().map(|group| group.num_units).sum();
    println!("{}", remaining);
}
